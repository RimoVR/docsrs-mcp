...
class TestPopularCratesManager:
    ...
    async def manager(self, tmp_path):
        ...
        with patch("docsrs_mcp.popular_crates.config.POPULAR_CRATES_CACHE_FILE", tmp_path / "cache.msgpack"):
...
class TestPreIngestionWorker:
    ...
    async def test_memory_monitoring(self, worker):
        ...
        import sys
        with patch.object(sys.modules["docsrs_mcp.popular_crates"], "psutil", create=True) as mock_psutil:
