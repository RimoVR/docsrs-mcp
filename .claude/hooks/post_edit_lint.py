    ...
    if file_type == 'rust':
        ...
        if check_tool_available('npm') or check_tool_available('npx'):
            try:
                ...
                eslint_config_exists = any((project_root / f).exists() for f in 
                ...
                if eslint_config_exists:
                    ...
                    if lint_result.returncode != 0:
                        results.append({
                            'tool': 'eslint',
                            'status': 'error',
                            'message': lint_result.stdout[:1000] if lint_result.stdout else lint_result.stderr[:1000]
                        })
                
                # Run TypeScript compiler for .ts/.tsx files
                if file_type == 'typescript' and (project_root / 'tsconfig.json').exists():
                ...
                prettier_config_exists = any((project_root / f).exists() for f in 
        ...
        try:
            ...
            if check_tool_available('mypy'):
                ...
                project_root = Path(file_path).parent
                while project_root != project_root.parent:
                    if any((project_root / f).exists() for f in ['pyproject.toml', 'setup.py', 'mypy.ini']):
                        break
                    project_root = project_root.parent
    ...
    try:
        ...
        if all_results:
            ...
            for result in all_results:
                status_marker = "[ERROR]" if result['status'] == 'error' else "[WARNING]"
