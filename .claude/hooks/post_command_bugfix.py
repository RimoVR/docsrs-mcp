...
def extract_error_info(command, output):
    ...
    if 'cargo' in command:
        ...
        if 'clippy' in command and ('warning' in output_lower or 'error' in output_lower):
...
def main():
    ...
    try:
        ...
        bugfix_commands = ['cargo build', 'cargo test', 'cargo clippy', 'npm test', 
                          'npm run lint', 'pytest', 'make', 'docker build']
