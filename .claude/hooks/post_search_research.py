...
def is_relevant_finding(search_query, search_results, relevance_score_out):
    ...
    relevance_indicators = [
        'version', 'v2', 'v3', 'breaking change', 'deprecated',
        'security', 'vulnerability', 'CVE', 'patch',
        'performance', 'optimization', 'benchmark',
        'best practice', 'recommended', 'anti-pattern',
        'bug', 'issue', 'workaround', 'fix',
        'update', 'release', 'announcement',
        'experimental', 'beta', 'alpha', 'preview'
    ]
    ...
    return relevance_score > 2 or any(term in results_lower for term in ['2024', '2025', 'latest'])
...
def update_research_findings(finding_data):
    ...
    if findings_path.exists():
        ...
        findings = {
            "findings": [],
            "lastUpdated": "",
            "categories": ["libraries", "patterns", "solutions", "version-specific", "security"]
        }
    ...
    try:
        ...
        if is_relevant_finding(search_query, tool_output, relevance_score):
            finding = {
                ...
                "category": "version-specific" if any(y in search_query.lower() for y in ['2024', '2025', 'version']) else "general",
                "relevantInfo": tool_output[:500] if isinstance(tool_output, str) else str(tool_output)[:500],
                "fullContext": {
            ...
            print(f"Added search findings for '{search_query}' to ResearchFindings.json (relevance: {relevance_score[0]})")
