...
def analyze_edit_significance(file_path, edit_content):
    ...
    significance_indicators = {
        'high': [
            'pub fn', 'pub struct', 'pub trait', 'pub enum',  # Rust public API
            'export', 'interface', 'type', 'class',  # TypeScript exports
            'async fn', 'impl', 'derive',  # Rust patterns
            'useState', 'useEffect', 'component',  # React patterns
            'CREATE', 'ALTER', 'TABLE',  # SQL
            'breaking', 'deprecated', 'BREAKING CHANGE',  # Version markers
        ],
        'medium': [
            'TODO', 'FIXME', 'HACK', 'BUG',  # Code markers
            'error', 'panic', 'unwrap', 'expect',  # Error handling
    ...
    high_score = sum(1 for indicator in significance_indicators['high'] 
                    if indicator.lower() in content_lower)
    
    # Check for medium significance indicators
    medium_score = sum(1 for indicator in significance_indicators['medium'] 
                      if indicator.lower() in content_lower)
    
    # File type significance
    path = Path(file_path)
    is_api_file = any(part in path.parts for part in ['api', 'routes', 'handlers', 'services'])
    is_config_file = path.name in ['Cargo.toml', 'package.json', '.env', 'config.rs', 'config.ts']
    
    return {
        'is_significant': high_score > 0 or medium_score > 2 or is_api_file or is_config_file,
