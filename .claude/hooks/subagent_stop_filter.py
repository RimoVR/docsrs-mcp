...
def extract_relevant_content(transcript: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for entry in transcript:
        if entry.get("role") == "user" and not original_task:
            ...
            if any(
                ...
                for keyword in ["found", "discovered", "located", "identified"]
            ):
                ...
                for sentence in sentences:
                    if any(
                        ...
                        for kw in ["found", "discovered", "located", "identified"]
                    ):
            ...
            # Track important commands
                ...
                if command and not any(
                    skip in command for skip in ["ls", "pwd", "cd", "echo"]
                ):
    ...
    if result["key_findings"]:
        result["summary"] = (
            f"Found {len(result['key_findings'])} relevant items for: {original_task}"
        )
    elif result["errors"]:
        result["summary"] = (
            f"Encountered {len(result['errors'])} errors while: {original_task}"
        )
    elif result["files_modified"]:
        result["summary"] = (
            f"Modified {len(result['files_modified'])} files for: {original_task}"
        )
    else:
...
def main():
    ...
    try:
        ...
        output = {"continue": True, "filtered_summary": filtered_data}
