#!/usr/bin/env python3
"""Test serde:latest ingestion with trait extraction fixes."""
...
async def test_serde_latest():
    """Test serde:latest ingestion with the trait extraction fixes."""
    
    print("🧪 Testing serde:latest ingestion with trait extraction fixes...")
    
    # Remove existing serde latest cache to force fresh ingestion
    latest_db = Path("./cache/serde/latest.db")
    
    if latest_db.exists():
        latest_db.unlink()
        print(f"🗑️ Removed existing database: {latest_db}")
    
    try:
        from src.docsrs_mcp.ingest import ingest_crate
        
        print(f"🚀 Ingesting serde latest (this should have rustdoc JSON)...")
        db_path = await ingest_crate("serde", "latest")
        ...
        with sqlite3.connect(db_path) as conn:
            ...
            if trait_count > 0:
                ...
                # Sample the new data
                print(f"\n📋 Sample of trait implementations:")
                cursor = conn.execute("""
                ...
                print("   ❌ No trait implementations found - check if rustdoc JSON was used")
                return False
...
if __name__ == "__main__":
    success = asyncio.run(test_serde_latest())
    if success:
        print("\n🎉 Latest version test completed!")
    else:
        print("\n💥 Latest version test failed!")